...
- **vLLM** (optional): Only needed for serving tests (`-k vllm`)
...
# Skip slow models (DeepSeek 671B, Kimi 1T)
HF_HUB_CACHE=~/huggingface/hub pytest tests/weights/gpu/ -v --ignore=tests/weights/gpu/test_deepseek.py --ignore=tests/weights/gpu/test_kimi.py
```

## Coverage matrix

| File | Model | Merge | Adapter | FP8 Quant | INT4 | GPU accel |
|------|-------|:-----:|:-------:|:---------:|:----:|:---------:|
| test_qwen3.py | Qwen3-4B | x | x | | | |
| test_qwen3_5.py | Qwen3.5-35B-A3B | x | x | x | | |
| test_deepseek.py | DeepSeek-V3.1 | | | x | | x |
| test_kimi.py | Kimi-K2-Thinking | x | x | | x | |
| test_gpt_oss.py | GPT-OSS-20B | x | x | | | |
