...
**On-policy** (:mod:`~tinker_cookbook.distillation.train_on_policy`):
    ...
    Best when you want the student to explore its own distribution while being
...
**Off-policy** (:mod:`~tinker_cookbook.distillation.train_off_policy`):
    ...
    Supports single-teacher and multi-teacher (MOPD) configurations.
...
**SDFT** (:mod:`~tinker_cookbook.distillation.sdft`):
    ...
    on-policy. Combines on-policy generation with demonstration-conditioned
    teacher scoring.
...
point. Access them via the submodule to avoid naming conflicts::
    ...
    await train_on_policy.main(cfg, dataset)

Shared dataset utilities (:class:`DistillationDatasetConfig`,
:class:`TeacherConfig`, :class:`PromptOnlyDatasetBuilder`) and the SDFT
protocol (:class:`SDFTBatchProvider`) are re-exported here for convenience.
"""
...
from tinker_cookbook.distillation.sdft import SDFTBatchProvider, build_sdft_teacher_prompt

__all__ = [
    # Submodules (access Config + main() via submodule)
    "train_on_policy",
    "train_off_policy",
    "sdft",
