...
Base `Qwen/Qwen3.5-4B` vs the same model after 30 GRPO steps under the fixed reward (grade `solve(...)` on hidden inputs). Unlike Experiment 1, after training the model writes real RILL functions, not constant emits.
