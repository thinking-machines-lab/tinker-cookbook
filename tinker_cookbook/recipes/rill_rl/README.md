...
credit), giving gradient even early when exact-match is all-zero on a fresh DSL (see
[`training/grading.py`](./training/grading.py)). Tasks are generated by writing a reference
solution *in RILL* and using its interpreter output as the exact expected answer, and we
hold out **disjoint task families** for eval to measure transfer, not memorization (see
[`training/tasks.py`](./training/tasks.py)). The trainer holds the expected outputs; the
app never sees them.
