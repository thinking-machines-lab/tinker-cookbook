...
from tinker_cookbook.recipes.fpgb.loss import compute_fpgb_loss
...
def compute_post_fpgb_metrics(
    reference_data: list[tinker.Datum],
    post_logprobs: list[torch.Tensor],
    beta: float,
) -> dict[str, float]:
    """Measure the realized FPGB update after the optimizer step.

    The rollout log-probabilities stored in ``reference_data`` represent
    ``pi_k``. ``post_logprobs`` are recomputed on the exact same sampled
    tokens after ``optim_step`` and therefore represent ``pi_{k+1}``.

    We compare

        log pi_{k+1}(a|s) - log pi_k(a|s)

    against the fixed FPGB regression target ``beta * advantage``.
    """
    if len(reference_data) != len(post_logprobs):
        raise ValueError(
            "FPGB post-update diagnostic received mismatched batch sizes: "
            f"{len(reference_data)=}, {len(post_logprobs)=}"
        )

    deltas: list[torch.Tensor] = []
    targets: list[torch.Tensor] = []

    for datum, current_logp in zip(reference_data, post_logprobs, strict=True):
        old_logp = datum.loss_fn_inputs["logprobs"].to_torch().to(
            device=current_logp.device, dtype=current_logp.dtype
        )
        advantages = datum.loss_fn_inputs["advantages"].to_torch().to(
            device=current_logp.device, dtype=current_logp.dtype
        )
        mask = datum.loss_fn_inputs["mask"].to_torch().to(
            device=current_logp.device, dtype=current_logp.dtype
        )
        ...
        deltas.append((current_logp - old_logp)[active])
        targets.append((beta * advantages)[active])

    if not deltas:
        return {
            "fpgb/post_delta_mean": 0.0,
            "fpgb/post_delta_std": 0.0,
            "fpgb/post_target_mean": 0.0,
            "fpgb/post_target_std": 0.0,
            "fpgb/post_residual_rmse": 0.0,
            "fpgb/post_delta_target_cosine": 0.0,
        }
    ...
    if delta_norm.item() > 0.0 and target_norm.item() > 0.0:
        cosine = torch.dot(delta, target) / (delta_norm * target_norm)
        cosine_value = cosine.item()
    else:
        cosine_value = 0.0

    return {
        "fpgb/post_delta_mean": delta.mean().item(),
        "fpgb/post_delta_std": delta.std(unbiased=False).item(),
        "fpgb/post_target_mean": target.mean().item(),
        "fpgb/post_target_std": target.std(unbiased=False).item(),
        "fpgb/post_residual_rmse": residual.square().mean().sqrt().item(),
        "fpgb/post_delta_target_cosine": cosine_value,
    }

def _to_fpgb_custom_datum(datum: tinker.Datum) -> tinker.Datum:
    """Create the restricted Datum accepted by forward_backward_custom.

    Tinker's custom-loss API currently accepts only `target_tokens` and
    `weights`. FPGB rollout logprobs, advantages, and masks stay client-side
    in the original Datum and are captured by the custom-loss closure.

    We use the action mask as `weights` for the surrogate forward pass.
    The custom-loss implementation later replaces these weights with the
    exact client-computed gradient dL/dlogp during backward propagation.
    """
    required = {"target_tokens", "logprobs", "advantages", "mask"}
    missing = required - set(datum.loss_fn_inputs)
    if missing:
        raise ValueError(
            f"FPGB Datum is missing required loss inputs: {sorted(missing)}"
        )
...
async def train_step(
    ...
    """Train one FPGB step on collected trajectories.

    FPGB fits the policy-function residual

        delta_logp = log pi_theta(a|s) - log pi_k(a|s)

    toward the functional-gradient target

        fpgb_beta * advantage.

    The rollout log-probabilities stored in each Datum represent the
    reference policy pi_k. The differentiable log-probabilities returned
    by Tinker's custom forward/backward call represent the current model.

    Args:
        data_D:
            Training data assembled from trajectory rollouts. Each Datum
            contains rollout log-probabilities, advantages, and action masks.
        training_client:
            Client connected to the Tinker training service.
        learning_rate:
            Adam learning rate.
        num_substeps:
            Number of sub-batches to split data_D into. Each sub-batch
            performs one FPGB forward/backward pass followed by one
            optimizer step.
        fpgb_beta:
            Functional FPGB step size. The regression target is

                fpgb_beta * advantage.

        metrics:
            Optional dictionary updated with optimizer metrics from the
            final optimizer step.

    Returns:
        Per-datum training log-probabilities returned by the model forward
        passes, in the same order as data_D.
    """
    batches = split_list(
        data_D,
        min(num_substeps, len(data_D)),
    )
    ...
    for batch in batches:
        # Tinker's custom-loss API accepts only target_tokens and weights in
        # the submitted Datum. Keep the FPGB rollout statistics in `batch`
        # and capture them in the local Python closure.
        custom_batch = [_to_fpgb_custom_datum(d) for d in batch]

        fpgb_loss_fn = partial(
            compute_fpgb_loss,
            beta=fpgb_beta,
        ...
        training_logprobs_D.extend(
            _training_logprobs_from_fwd_bwd(fwd_bwd_result)
        )

        # Propagate custom FPGB loss metrics to the outer training metrics.
        if metrics is not None and fwd_bwd_result.metrics:
        ...
        # ---------------------------------------------------------
        # FPGB post-update diagnostic
        # ---------------------------------------------------------
        # Re-evaluate the SAME sampled tokens only after the optimizer
        # update.  This measures the realized stage update
        #
        #   delta_post = log pi_{k+1}(a|s) - log pi_k(a|s)
        #
        # against the fixed target beta * A_k.  forward_async is
        # forward-only, so this does not apply another gradient update.
        post_fwd_future = await training_client.forward_async(
            custom_batch,
            loss_fn="cross_entropy",
        )
        post_fwd_result = await post_fwd_future.result_async()
        post_logprobs = _training_logprobs_from_fwd_bwd(post_fwd_result)

        if metrics is not None:
            metrics.update(
                compute_post_fpgb_metrics(
                    reference_data=batch,
                    post_logprobs=post_logprobs,
                    beta=fpgb_beta,
                )
            )
...
class Config:
    ...
    # -------------------------------------------------------------------------
    # FPGB and optimizer behavior
    # -------------------------------------------------------------------------
    # Functional policy-gradient boosting step size.  The residual fitting
    # target for each sampled action token is
    #
    #     delta_logp ~= fpgb_beta * advantage.
    #
    # This is the beta in the FPGB functional update, not a KL penalty
    # coefficient.
    fpgb_beta: float = 0.1

    # Legacy generic-RL loss fields retained temporarily so copied recipe
    # configuration remains source-compatible.  The synchronous FPGB path
        ...
        with trace.trace_iteration(step=i_batch) as window:
            ...
            if batch_skipped:
                ...
                if config.remove_constant_reward_groups:
                    filtered_pairs = [
                        (env_group_builder, trajectory_group)
                        for env_group_builder, trajectory_group in safezip(
                            env_group_builders_P, trajectory_groups_P
                        )
                        if not all_same(trajectory_group.get_total_rewards())
                    ]

                    if filtered_pairs:
                        env_group_builders_P = [pair[0] for pair in filtered_pairs]
                        trajectory_groups_P = [pair[1] for pair in filtered_pairs]
                    else:
                        logger.warning(
                            "All rewards are uniform. There will be no gradient"
                        )
                        # Preserve the existing non-empty-list behavior while
                        # keeping builders and trajectory groups aligned.
                        env_group_builders_P = env_group_builders_P[:1]
                        trajectory_groups_P = trajectory_groups_P[:1]
...
async def main(
    ...
    Example::
        ...
        config = Config(
            ...
            max_tokens=2048,
            fpgb_beta=0.1,
            log_path="./logs/my_fpgb_run",
        )
        asyncio.run(main(config=config))
    """

    if config.fpgb_beta <= 0:
        raise ConfigurationError(
            f"fpgb_beta must be positive, got {config.fpgb_beta}"
        )
    ...
    # Training loop
    #
    # FPGB V0 intentionally supports only the fully synchronous path.  The
    # async and streaming-minibatch implementations below still use Tinker's
    # built-in forward_backward_async loss path, which would silently train a
    # policy-gradient objective instead of the FPGB residual-regression
    # objective.  Fail fast until those paths are converted explicitly.
    if config.async_config is not None:
        raise ConfigurationError(
            "FPGB V0 does not support async_config yet; use synchronous training."
        )
    if config.stream_minibatch_config is not None:
        raise ConfigurationError(
            "FPGB V0 does not support stream_minibatch_config yet; "
