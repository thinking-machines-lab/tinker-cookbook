...
async def cli_main(cli_config: CLIConfig):
    ...
    if cli_config.log_path is not None:
        ...
        log_path = f"/tmp/tinker-examples/math_rl/{cli_config.env}-{cli_config.model_name}-{cli_config.lora_rank}rank-{cli_config.learning_rate}lr-{datetime.now().strftime('%Y-%m-%d-%H-%M')}"
    # Create full config
    config = Config(
