...
- Task loading: load_deepcoder_tasks() using existing code_rl infrastructure
- Environment construction: build_env(), EnvGroupBuilder, RLDataset, CodeDatasetBuilder
"""
...
from tinker_cookbook.recipes.code.tools import CodeReward, CodeTask, CodeTool
...
class RLDataset(types.RLDataset):
    ...
    def __len__(self) -> int:
        # Ceiling division to include partial batches (matches old code_rl behavior)
        return (len(self.env_group_builders) + self.batch_size - 1) // self.batch_size


@chz.chz
class CodeDatasetBuilder(types.RLDatasetBuilder):
    ...
    async def __call__(self) -> tuple[types.RLDataset, types.RLDataset | None]:
        ...
        # Load test tasks (group_size=1 for eval, matching old code_rl behavior)
