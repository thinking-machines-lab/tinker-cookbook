...
so the policy gets gradient even before it is fully correct. ``CogTask`` lives in
...
from tinker_cookbook.recipes.cog_rl.agent_app.cog_lang import Result, run_cog
...
def shaped_reward(program: str, task: CogTask) -> tuple[float, dict]:
