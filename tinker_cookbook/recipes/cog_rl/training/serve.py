"""Serve the Cog agent app backed by a Tinker policy, with live checkpoint switching.
...
endpoints to the app so you can A/B test checkpoints live in the UI:
    ...
    GET  /api/checkpoints        list the project's Cog checkpoints (+ the base model)
...
from tinker_cookbook.recipes.cog_rl.agent_app import server as appserver
from tinker_cookbook.recipes.cog_rl.training.checkpoints import list_project_checkpoints
from tinker_cookbook.recipes.cog_rl.training.proxy import SamplingProxy
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Serve the Cog agent with live checkpoint A/B.")
    ap.add_argument("--project", default=os.environ.get("COG_PROJECT_ID"))
    ...
    print(
        f"Cog agent on :{args.app_port} (proxy :{args.proxy_port}); "
