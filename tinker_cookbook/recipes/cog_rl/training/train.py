"""GRPO post-training for the Cog agent app, via the sampling proxy.
...
Example::
    ...
    python -m tinker_cookbook.recipes.cog_rl.agent_app.server
...
from tinker_cookbook.recipes.cog_rl.training import checkpoints
...
class Config:
    ...
    log_path: str = "/tmp/tinker-examples/cog_rl"
    save_every: int = 20
...
async def _run_batch_rollouts(
    cfg: Config, tasks: list[CogTask], batch_idx: int, proxy_base: str
...
def _batch_tasks(config: Config, train_tasks: list[CogTask], batch_idx: int) -> list[CogTask]:
...
def _setup(config: Config):
    ...
    service = tinker.ServiceClient(
        ...
        user_metadata=recipe_user_metadata("recipe_cog_rl"),
    )
