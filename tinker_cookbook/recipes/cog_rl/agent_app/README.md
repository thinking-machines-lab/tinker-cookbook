# Cog Coding Agent (standalone app)

A self-contained AI coding agent for **Cog**, a small out-of-distribution DSL. Given a
task, the agent asks an OpenAI chat model to write a Cog program, runs it through the
...
- `GET /` — the web UI.
- `GET /playground` — a Cog interpreter/REPL page for debugging programs (run a program,
...
- `agent.py` — `CogAgent`: the chat loop with interpreter-driven self-correction.
- `program.py` — extract the Cog program from a model message.
- `prompts.py` — the Cog language reference shown to the model.
- `cog_lang/` — the vendored Cog interpreter and grammar (the language spec).
