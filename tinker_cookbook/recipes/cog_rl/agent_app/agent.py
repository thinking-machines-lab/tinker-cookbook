"""The Cog coding agent.

A plain chat agent: given a task, it asks an OpenAI chat model to write a Cog program,
...
def _error_feedback(error: str) -> str:
    ...
    hint = {
        "parse": (
            "Re-check Cog syntax: assignment is `value -> name`, equality is a single "
    ...
    return (
        ...
        "Return the full corrected Cog program in a ```cog code block."
    ).strip()
