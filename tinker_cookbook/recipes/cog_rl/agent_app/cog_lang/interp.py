"""
Cog interpreter — a deterministic tree-walking evaluator.

Public API:
    run_cog(src: str, *, max_steps: int = 100_000) -> Result
...
_GRAMMAR = (Path(__file__).parent / "cog.lark").read_text()
...
class CogError(Exception):
    """Runtime error inside a Cog program."""
    ...
    def get(self, name):
        ...
        while e is not None:
            if name in e.vars:
                return e.vars[name]
            e = e.parent
        raise CogError(f"unbound name '{name}'")
...
class Interpreter:
    ...
    def _tick(self):
        ...
        if self.steps > self.max_steps:
            raise CogError("step budget exceeded")  # caught & retagged as budget
    ...
    def _exec(self, node: Tree, env: _Env):
        ...
        if kind == "bind":
            ...
            if not isinstance(seq, list):
                raise CogError("'walk across' expects a list")
            for item in seq:
                self._tick()
                loop_env = _Env(env)
            ...
            raise CogError(f"unknown statement: {kind}")
    ...
    def _eval(self, node, env: _Env):
        if isinstance(node, Token):
            raise CogError(f"stray token {node!r}")
        d = node.data
        ...
        if d == "div":
            ...
            if b == 0: raise CogError("division by zero")
            a = _num(self._eval(node.children[0], env))
            return a // b if isinstance(a, int) and isinstance(b, int) else a / b
        if d == "mod":
            b = _num(self._eval(node.children[1], env))
            if b == 0: raise CogError("modulo by zero")
        ...
        raise CogError(f"unknown expression: {d}")
    ...
    def _eval_call(self, node: Tree, env: _Env):
        ...
        if not isinstance(value, _Func):
            raise CogError("attempt to call a non-function")
...
def _num(v):
    if isinstance(v, bool) or not isinstance(v, (int, float)):
        raise CogError(f"expected a number, got {_type(v)}")
    return v
...
def _compare(op, a, b):
    ...
    if isinstance(a, str) and isinstance(b, str):
        ...
        raise CogError(f"cannot order {_type(a)} and {_type(b)}")
    return {"lt": a < b, "gt": a > b, "le": a <= b, "ge": a >= b}[op]

def _index(target, idx):
    if isinstance(target, (list, str)):
        if not isinstance(idx, int) or isinstance(idx, bool):
            raise CogError("index must be an integer")
        if idx < 0 or idx >= len(target):
            raise CogError(f"index {idx} out of range (size {len(target)})")
        return target[idx]
    raise CogError(f"cannot index {_type(target)}")
...
def _b_count(it, args):
    ...
    raise CogError("count expects a list or text")
...
def _b_range(it, args):
    ...
    raise CogError("range expects 1 or 2 ints")

def _b_push(it, args):
    xs, v = _arity(args, 2, "push")
    if not isinstance(xs, list): raise CogError("push expects a list")
    return xs + [v]

def _b_head(it, args):
    (xs,) = _arity(args, 1, "head")
    if not isinstance(xs, list) or not xs: raise CogError("head of empty/non-list")
    return xs[0]

def _b_tail(it, args):
    (xs,) = _arity(args, 1, "tail")
    if not isinstance(xs, list): raise CogError("tail expects a list")
    return xs[1:]

def _b_slice(it, args):
    xs, a, b = _arity(args, 3, "slice")
    if not isinstance(xs, (list, str)): raise CogError("slice expects list/text")
    _need_int(a, "slice"); _need_int(b, "slice")
    return xs[a:b]

def _b_upper(it, args):
    (s,) = _arity(args, 1, "upper")
    if not isinstance(s, str): raise CogError("upper expects text")
    return s.upper()

def _b_lower(it, args):
    (s,) = _arity(args, 1, "lower")
    if not isinstance(s, str): raise CogError("lower expects text")
    return s.lower()

def _b_chars(it, args):
    (s,) = _arity(args, 1, "chars")
    if not isinstance(s, str): raise CogError("chars expects text")
    return list(s)

def _b_join(it, args):
    xs, sep = _arity(args, 2, "join")
    if not isinstance(xs, list) or not isinstance(sep, str):
        raise CogError("join expects (list, text)")
...
def _b_int(it, args):
    (x,) = _arity(args, 1, "int")
    if isinstance(x, bool): raise CogError("int expects number/text")
    if isinstance(x, (int, float)): return int(x)
    if isinstance(x, str):
        try: return int(x)
        except ValueError: raise CogError(f"cannot read int from {x!r}")
    raise CogError("int expects number/text")

def _arity(args, n, name):
    if len(args) != n:
        raise CogError(f"{name} expects {n} arg(s), got {len(args)}")
    return args

def _need_int(v, name):
    if not isinstance(v, int) or isinstance(v, bool):
        raise CogError(f"{name} expects integer(s)")
...
def run_cog(src: str, *, max_steps: int = 100_000) -> Result:
    try:
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 2:
        print("usage: python interp.py <file.cog>")
        sys.exit(2)
    res = run_cog(Path(sys.argv[1]).read_text())
    if res.output:
        print(res.output)
    if not res.ok:
