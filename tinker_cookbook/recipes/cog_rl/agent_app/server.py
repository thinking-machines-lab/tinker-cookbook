"""FastAPI backend + UI for the Cog coding agent.
...
def _agent(req: SolveRequest) -> CogAgent:
    return CogAgent(
        model=req.model,
...
async def playground() -> FileResponse:
    """A Cog interpreter/REPL page for debugging programs."""
    return FileResponse(_STATIC / "playground.html")


class RunCogRequest(BaseModel):
    program: str
    max_steps: int = 100_000


@app.post("/api/run_cog")
async def run_cog_endpoint(req: RunCogRequest) -> dict:
    """Run a Cog program through the reference interpreter (no model). For debugging."""
    try:
        res = run_cog(req.program, max_steps=req.max_steps)
        return {
            "ok": res.ok,
            "output": res.output,
            "trace": res.trace,
        ...
        return {
            ...
            "output": "",
            "trace": "",
            "error": f"runtime:{type(e).__name__}",
            "steps": 0,
        }
...
def main() -> None:
    ...
    uvicorn.run(
        app,
        host=os.environ.get("COG_HOST", "0.0.0.0"),
