...
def merge_adapter_weights(base_model: torch.nn.Module, adapter_weights: dict[str, torch.Tensor], config: dict):
    ...
    for n in adapter_weight_names:
        ...
        if ".experts" not in n:
            ...
            assert merged_lora.shape == model_state_dict[target_key].shape, (n, merged_lora.shape, model_state_dict[target_key].shape)
...
def main():
    ...
    parser.add_argument("--tinker-adapter-path", type=str, required=True, help="Path to the Tinker adapter")
    parser.add_argument("--hf-model", type=str, required=True, help="HuggingFace model name or path")
    parser.add_argument("--output-path", type=str, required=True, help="Path to save the merged model")
