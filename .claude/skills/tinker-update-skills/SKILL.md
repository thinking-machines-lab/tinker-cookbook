...
Tinker skills live in this repo at `.claude/skills/`. The install script (`.claude/install-skills.sh`) symlinks them into `~/.claude/skills/` so they're available in any project. All skill directories are already prefixed with `tinker-`.
...
Dev-only skills (`tinker-ci`, `tinker-contributing`, `tinker-new-recipe`, `tinker-manage-skills`) are excluded from global install — they're only available when working inside the tinker-cookbook repo itself.
