...
For most use cases, you don't need to manually pad tokens. The cookbook provides higher-level abstractions:

- **Renderers**: `Qwen3VLRenderer` and `Qwen3VLInstructRenderer` handle vision special tokens automatically. See [Rendering: Vision Inputs](/rendering#vision-inputs).

- **VLM Classifier recipe**: The <CookbookLink path="tinker_cookbook/recipes/vlm_classifier">VLM Classifier recipe</CookbookLink> shows a complete example of training a VLM image classifier, including data preparation and evaluation.
