...
The `SamplingClient` does not provide direct access to a `get_tokenizer()` method. To tokenize inputs, use one of these approaches:
...
**Computing vision token count:** Qwen3-VL uses a ~32x compression ratio (`patch_size=16`, `merge_size=2`), so:

```
visual_tokens ≈ (H × W) / (32 × 32) = (H × W) / 1024
```

For a 448×448 image: `448 × 448 / 1024 = 196 tokens`. The actual count may differ if preprocessing resizes or pads the image—use the image processor for exact values:
...
**Example with fixed 448×448 resolution:**

```python
# Qwen3-VL at 448x448: (448*448)/1024 = 196 tokens
