...
The `SamplingClient` does not provide direct access to a `get_tokenizer()` method.

To tokenize inputs, use one of these approaches:
...
When training VLMs, `ImageChunk`s are expanded into vision tokens by the model. The `weights` and `target_tokens` arrays in `loss_fn_inputs` must account for these tokens to match the server-side sequence length.
...
Alternative, you can get `vision_token_count` using the `ImageProcessor` class helper:

Qwen3-VL uses a ~32x compression ratio (`patch_size=16`, `merge_size=2`), so:

```
visual_tokens ≈ (H × W) / (32 × 32) = (H × W) / 1024
```

Making this calculation in code looks like this:
