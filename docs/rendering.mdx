...
The default renderers (`qwen3`, `llama3`, `deepseekv3`, etc.) match HF behavior. The one exception is `Qwen3Renderer` with `strip_thinking_from_history=False`, which is a special mode for multi-turn RL efficiency that does NOT match HF (see [Sequence Extension](/rl/sequence-extension)).
